...
export function searchNewGame( userId ) {
  ...
  return ( dispatch, getState ) => {
    ...
    /*
     ...
     1º) If nobody is waiting for a game, then you'll be that one.
     2º) If there is already somebody, you'll be on a temporal sub-matchmaking list
          in order to avoid concurrence, ensuring only one person is getting matched
          with another one.
     3º) The matchmaking criteria is the autogenerated timestamp-based push's Id.
          He who cames the first, should be the first, regadless the biblia! >:C
     4º) To ensure nobody access to the board, generating glitches, the waiter and it's
